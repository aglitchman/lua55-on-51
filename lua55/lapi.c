...
*/
static TValue *index2value (lua55_State *L, int idx) {
  CallInfo *ci = L->ci;
  if (idx > 0) {
...
*/
static StkId index2stack (lua55_State *L, int idx) {
  CallInfo *ci = L->ci;
  if (idx > 0) {
    StkId o = ci->func.p + idx;
...
LUA_API lua_CFunction lua55_atpanic (lua55_State *L, lua_CFunction panicf) {
  lua_CFunction old;
  lua_lock(L);
  old = G(L)->panic;
  G(L)->panic = panicf;
...
LUA_API lua_Number lua55_version (lua55_State *L) {
  UNUSED(L);
...
*/
static void reverse (lua55_State *L, StkId from, StkId to) {
  for (; from < to; from++, to--) {
    TValue temp;
...
LUA_API const char *lua55_typename (lua55_State *L, int t) {
  UNUSED(L);
...
LUA_API int lua55_rawequal (lua55_State *L, int index1, int index2) {
...
LUA_API int lua55_compare (lua55_State *L, int index1, int index2, int op) {
  const TValue *o1;
  const TValue *o2;
  int i = 0;
...
LUA_API lua_Number lua55_tonumberx (lua55_State *L, int idx, int *pisnum) {
  lua_Number n = 0;
  const TValue *o = index2value(L, idx);
...
LUA_API lua_Integer lua55_tointegerx (lua55_State *L, int idx, int *pisnum) {
  lua_Integer res = 0;
  const TValue *o = index2value(L, idx);
...
LUA_API lua_Unsigned lua55_rawlen (lua55_State *L, int idx) {
  const TValue *o = index2value(L, idx);
  switch (ttypetag(o)) {
...
LUA_API lua_CFunction lua55_tocfunction (lua55_State *L, int idx) {
  const TValue *o = index2value(L, idx);
...
LUA_API void lua55_pushnumber (lua55_State *L, lua_Number n) {
  lua_lock(L);
...
LUA_API void lua55_pushinteger (lua55_State *L, lua_Integer n) {
  lua_lock(L);
...
LUA_API void lua55_pushcclosure (lua55_State *L, lua_CFunction fn, int n) {
  lua_lock(L);
  if (n == 0) {
...
static int auxgetstr (lua55_State *L, const TValue *t, const char *k) {
  lu_byte tag;
...
** cannot collect it.
*/
static void getGlobalTable (lua55_State *L, TValue *gt) {
...
LUA_API int lua55_geti (lua55_State *L, int idx, lua_Integer n) {
  TValue *t;
  lu_byte tag;
  lua_lock(L);
  t = index2value(L, idx);
...
static int finishrawget (lua55_State *L, lu_byte tag) {
...
l_sinline Table *gettable (lua55_State *L, int idx) {
...
LUA_API int lua55_rawgeti (lua55_State *L, int idx, lua_Integer n) {
  Table *t;
  lu_byte tag;
  lua_lock(L);
  t = gettable(L, idx);
...
LUA_API void lua55_createtable (lua55_State *L, int narray, int nrec) {
  Table *t;
  lua_lock(L);
  t = luaH_new(L);
...
*/
static void auxsetstr (lua55_State *L, const TValue *t, const char *k) {
  int hres;
  TString *str = luaS_new(L, k);
  api_checkpop(L, 1);
...
LUA_API void lua55_seti (lua55_State *L, int idx, lua_Integer n) {
  TValue *t;
  int hres;
  lua_lock(L);
  api_checkpop(L, 1);
...
static void aux_rawset (lua55_State *L, int idx, TValue *key, int n) {
  Table *t;
  lua_lock(L);
  api_checkpop(L, n);
  t = gettable(L, idx);
...
LUA_API void lua55_rawseti (lua55_State *L, int idx, lua_Integer n) {
  Table *t;
  lua_lock(L);
  api_checkpop(L, 1);
  t = gettable(L, idx);
...
static void f_call (lua55_State *L, void *ud) {
...
LUA_API int lua55_load (lua55_State *L, lua_Reader reader, void *data,
...
*/
LUA_API int lua55_dump (lua55_State *L, lua_Writer writer, void *data, int strip) {
  int status;
...
LUA_API lua_Alloc lua55_getallocf (lua55_State *L, void **ud) {
  lua_Alloc f;
  lua_lock(L);
  if (ud) *ud = G(L)->ud;
  f = G(L)->frealloc;
...
LUA_API void lua55_setallocf (lua55_State *L, lua_Alloc f, void *ud) {
  lua_lock(L);
  G(L)->ud = ud;
  G(L)->frealloc = f;
  lua_unlock(L);
...
void lua55_setwarnf (lua55_State *L, lua_WarnFunction f, void *ud) {
  lua_lock(L);
  G(L)->ud_warn = ud;
  G(L)->warnf = f;
  lua_unlock(L);
...
void lua55_warning (lua55_State *L, const char *msg, int tocont) {
  lua_lock(L);
...
LUA_API void *lua55_newuserdatauv (lua55_State *L, size_t size, int nuvalue) {
  Udata *u;
  lua_lock(L);
...
static UpVal **getupvalref (lua55_State *L, int fidx, int n, LClosure **pf) {
